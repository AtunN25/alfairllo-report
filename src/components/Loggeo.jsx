...
function Loggeo() {
    const [formData, setFormData] = useState({
      ...
      const handleSubmit = async (e) => {
        ...
        if (!payload.date || isNaN(payload.from) || isNaN(payload.to) || isNaN(payload.well_id)) {
          alert("Todos los campos son obligatorios y deben ser válidos.");
          return;
        }
    
        try {
          const res = await fetch("http://localhost:3000/api/loggeo", {
            method: "POST",
          ...
          setFormData({ date: "", from: "", to: "", well_id: "" });
        } catch (error) {
  ...
  return (
    ...
    <h1 className="text-neutral-500 font-bold text-4xl">Logger Form</h1>
